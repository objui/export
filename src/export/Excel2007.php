...
class Excel2007 implements IExcel
{
    ...
    public function export()
    {
        ...
        for($i = 0;$i < count($table_header);$i++) {
            $letter_tmp = $letter[$i] . '1';
            $header_tmp = $table_header[$i];
            $excel->getActiveSheet()->setCellValue("$letter_tmp","$header_tmp");
        }
        ...
        for ($i = 2;$i <= count($table_data)+1;$i++) {
            ...
            foreach ($table_data[$i - 2] as $key=>$value) {
                $let_tmp = $letter[$j] . $i;
                $excel->getActiveSheet()->setCellValue("$let_tmp","$value");
                $j++;
            }
        }
